...
//   } else alert('C\'mon now. That number is outside of the specified range.');}
  ...
  while (correctAnswer === false && attempts > 0){
    let carQuestion= prompt('Can you name a vehicle that is made in America?');
    for (let index = 0; index < cars.length; index++){
      if(carQuestion === cars[index]){
        alert('You got it right.');
        console.log('checking cars', cars[index]);
        correctAnswer= true;
        break;
      }
    }
